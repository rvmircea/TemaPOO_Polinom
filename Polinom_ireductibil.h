...
public:
    ...
    Polinom_ireductibil(int nr, Monom* mono):Polinom(nr,mono){nr_monoame=nr,m=mono;}
    //aoel explicit constructor copiere din baza Polinom
    Polinom_ireductibil(const Polinom_ireductibil& p):Polinom(p){nr_monoame=p.nr_monoame;m=p.m;}
    ~Polinom_ireductibil();
